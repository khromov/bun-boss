...
- **`contractor.ts`** — schema **install and verify** on `start()`. Reads the target version from `package.json` → `bunboss.schema` and installs it fresh at that version, or migrates an older installed schema forward through `src/migrationStore.ts` (with `migrate: false` it verifies instead of installing or migrating). Also exposes the static `getConstructionPlans` used by `index.ts`.
