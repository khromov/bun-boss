...
- **The Bun-adapter traps are documented at their sites, not in one file.** Parameter-encoding fragility and the Bun 1.3.x pooled-connection leak live as WHY comments in `src/adapters/bun.ts` and the queue-cache comments in `src/manager.ts`; the sqlite driver quirks are enumerated in `docs/database-backends.md` and asserted by `scripts/spike-bun-sqlite.ts`. Read those before changing `src/adapters/bun.ts` or `src/adapters/sqlite.ts`, and keep them current when a Bun release fixes one.
