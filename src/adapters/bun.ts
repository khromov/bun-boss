...
// by the text binding.
//
// The rule is single: a json placeholder is recognised only by an explicit ::json/::jsonb cast, so
// every such placeholder in plans.ts must carry one (a bare `data @> $n` would be double-encoded).
// The plansSnapshot test guards the invariant at its source rather than growing this classifier.
    ...
    async executeSql (text: string, values?: unknown[]) {
      ...
      for (let attempt = 0; ; attempt++) {
        try {
          ...
          // transaction on a connection handed to us is bun 1.3.x leaking one into the pool (the
          // ABORTED_TRANSACTION comment above has the detail); 1.4 fixes it at the source.
